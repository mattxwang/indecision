...
type VTreeTypeLabels = 'LeftLinear' | 'RightLinear' | 'EvenSplit'
...
export default function VTreeSelect ({ setVTreeType }: Props): JSX.Element {
  const [selectedType, setSelectedType] = useState<VTreeTypeLabels>('RightLinear')
  ...
  return (<>
    <div className="inline-block relative mr-2">
      ...
      >
        {vtreeTypeOptions.map(option => <option value={option.value} key={option.label}>{option.label}</option>)}
      </select>
    ...
    {
      selectedType === 'EvenSplit' && <>num splits: <input
        value={numSplits}
        onChange={(e) => {
          const val = Number(e.target.value)
          if (isNaN(val) || val < 0) return
          setNumSplits(val)
          setVTreeType(selectedType === 'EvenSplit' ? { EvenSplit: val } : selectedType)
        }}
        className='input'
        type="number"
      /></>
